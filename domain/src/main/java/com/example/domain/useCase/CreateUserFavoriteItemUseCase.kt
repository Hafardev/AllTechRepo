...
class CreateUserFavoriteItemUseCase @Inject constructor(val userGitRepository: UserGitRepository) :
    BaseUseCase<UserRepositoryItemModel, Unit?>() {

    override suspend fun onExecute(param: UserRepositoryItemModel?) {
        param?.let { userGitRepository.createUserFavoriteListItem(it) }
    }

   /* override suspend fun onExecute(param: GetUserRepositoriesResultModel?) {
        param?.let { userGitRepository.createUserFavoriteListItem(it) }
    }*/
